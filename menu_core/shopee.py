...
# FORCE the profile directory to be the shared chrome_profile (tanpa per-account split)
orig_add_argument = Options.add_argument
def custom_add_argument(self, argument):
    if "--user-data-dir=" in argument:
        project_root = Path(__file__).resolve().parent.parent
        argument = f"--user-data-dir={project_root / 'data' / 'chrome_profile'}"
...
def extract_shopee_menu(store_metadata: dict, output_dir: str):
    ...
    # Gunakan Merchant Name dari GSheet sebagai target_name
    ...
    # Credentials — baca dari .env atau fallback ke default
    username = os.environ.get("SHOPEE_USERNAME", "allvbadmin")
    password = os.environ.get("SHOPEE_PASSWORD", "Shopee@321")

    # account_name digunakan sebagai identifier sesi (nama file session)
