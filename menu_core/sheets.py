...
CACHE_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "master_merchants_cache.csv")
...
def get_outlets_for_applicator(applicator_choice: str):
    ...
    for _, row in filtered_df.iterrows():
        ...
        # Ambil kredensial Grab (kolom "Nama Pengguna" / "Kata Sandi")
        grab_user = ""
        grab_pwd = ""
        if app_lower == 'grab':
            for suffix in [".1", ""]:
                col_u = f"Nama Pengguna{suffix}"
                col_p = f"Kata Sandi{suffix}"
                u_val = str(row.get(col_u, "")).strip()
                p_val = str(row.get(col_p, "")).strip()
                if u_val and u_val not in ("-", "nan", ""):
                    grab_user = u_val
                    grab_pwd = p_val
                    break
        ...
        outlets.append({
            ...
            'grab_username': grab_user,
            'grab_password': grab_pwd,
        })
