...
def load_gofood_session(identifier):
    ...
    json_file = MENU_DIR / "platforms" / "gofood" / "sessions" / f"session_gofood_{sanitized}.json"
    if not json_file.exists():
        phone_norm = normalisasi_nomor_hp(identifier)
        if phone_norm:
            json_file_old = MENU_DIR / "platforms" / "gofood" / "sessions" / f"session_{phone_norm}.json"
