...
def upload_file_to_gdrive(file_path: str, script_url: str, folder_id: str = None, sub_folder_name: str = None, outlet_folder_name: str = None) -> bool:
    """
    Mengirimkan file ke Google Apps Script Web App untuk disimpan di Google Drive.
    - sub_folder_name   : subfolder level 1 — nama platform (GR / S / GO)
    - outlet_folder_name: subfolder level 2 — nama outlet
    """
    path = Path(file_path)
    if not path.is_file():
    ...
    try:
        ...
        if outlet_folder_name:
            payload["outletFolderName"] = outlet_folder_name
...
def main():
    ...
    root_env = base_dir / ".env"   # /mnt/DATA/Proyek/Menu Outlet/.env
